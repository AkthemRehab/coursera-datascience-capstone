---
title: "Data Science Capstone Milestone Report - Datasets Exploratory Analysis for Natural Language Processing"
author: "YeongWei"
...
This section consist of information related to getting and cleaning the datasets followed by procedures taken to represent the datasets as useful R Objects.
...
1. Built `Corpus` as R Object with text documents described in [the Sample the Data section](#sample-the-data).
```{r constructCorpus}
...
ovid
```
2. Applied `Transformation` onto `Corpus` Object.
...
```
3. Built `TermDocumentMatrix` for evaluation of Term frequency by text documents.
...
termDocumentMatrix
```
4. Reduce sparsity from `TermDocumentMatrix`. This is removing thinly dispersed or scattered texts.
```{r removeSparsity}
...
A visual representation of 150 words that appears in the `Corpus` as brief linguistic context.
```{r createWordCloud}
...
## N-Gram(s)
N-Gram represents sequence of "n" number of text items(E.g. phonemes, syllables, letters, words, base pairs and etc). This section attempts to explore the 1-Gram, 2-Grams and 3-Grams of the created `Corpus`.
```{r createNgrams}
...
2. From [Word Cloud](#word-cloud). It was observed that there was not a determinable linguistic context (E.g. Scientific, Entertainment, Politics or etc ). Therefore it was assumed that the `Corpus` consist of a wide range of context.
3. From [N-Gram(s) section]. The Top 30 [1-Gram](#1-gram) and 2-Grams[#2-grams] appeared to have more reasonable linguistic context. 
...
2. Apply stricter text clearning process before building `Corpus`.
